...
const TermsAndConditions = () => {
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      try {
        ...
        if (isActive) {
          const terms = trust?.terms_content || '';
          setContent(terms);
        ...
        if (isActive) {
          setError('Terms & Conditions failed to load. Please try again later.');
        }
      } finally {
        if (isActive) {
          setLoading(false);
  ...
  const renderContent = () => {
    if (loading) {
      return <p className="text-gray-500">Loading terms & conditions...</p>;
    }
    if (error) {
      return <p className="text-red-600 font-medium">{error}</p>;
    }
    if (!content) {
      return <p className="text-gray-500">Terms & conditions content is not available.</p>;
    }
    return (
      <div
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <div className="text-white p-4 flex items-center gap-4"
        style={{ background: 'linear-gradient(90deg, #C0241A 0%, #2B2F7E 100%)' }}>
        <button onClick={() => navigate(-1)} className="p-1 rounded-lg" style={{ background: 'rgba(255,255,255,0.15)' }}>
          <ArrowLeft size={24} />
        </button>
        <h1 className="text-xl font-extrabold tracking-tight">Terms &amp; Conditions</h1>
      </div>

      <div className="flex-1 p-6 overflow-y-auto space-y-4 text-gray-700">
        {renderContent()}
        {updatedAt && !loading && !error && (
          <section className="pt-4" style={{ borderTop: '1px solid rgba(192,36,26,0.10)' }}>
